...
def activate_venv(path):
    ...
    if current_venv:
        print(f'You are currently in a virtual environment at "{current_venv}". Please exit with "deactivate" before activating a new one.')
        return
...
def create_venv(path):
    if os.path.isdir(path):
        print(f'A virtual environment already exists at "{path}". Delete the current one "rm -r {path}" before creating a new environment')
        return
